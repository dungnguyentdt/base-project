...
docker ps | grep my-frontend-app
...
| ---------------- | ----------------- | --------------------- |
| `CONTAINER_NAME` | `my-frontend-app` | Docker container name |
| `IMAGE_NAME`     | `my-frontend-app` | Docker image name     |
...
docker tag my-frontend-app:latest my-frontend-app:v1.2.0
```
...
docker inspect my-frontend-app --format='{{.State.Health.Status}}'

# Test application endpoint
curl http://localhost:3030

# View health check logs
docker inspect my-frontend-app --format='{{range .State.Health.Log}}{{.Output}}{{end}}'
```
...
docker stop my-frontend-app

# Start the application
docker start my-frontend-app

# Restart the application
docker restart my-frontend-app

# Remove the container completely
docker stop my-frontend-app && docker rm my-frontend-app
```
...
docker stats --no-stream my-frontend-app
```
...
docker logs --tail 100 my-frontend-app

# Check container status
docker ps | grep my-frontend-app

# Inspect container details
docker inspect my-frontend-app
```
...
docker build -f Dockerfile.dev -t my-frontend-app:dev .
docker run -d -p 3000:3000 -v $(pwd):/app --name my-frontend-app-dev my-frontend-app:dev
...
2. **Docker Build Fails**
   ...
   docker build --no-cache -t my-frontend-app:test .
   ```
...
3. **Container Won't Start**
   ...
   # Check container status
   docker ps -a | grep my-frontend-app

   # Inspect container configuration
   docker inspect my-frontend-app

   # Run container interactively for debugging
   docker run -it --rm my-frontend-app:latest sh
   ```
...
4. **Application Not Accessible**
   ...
   # Check port mapping
   docker port my-frontend-app

   # Test local connection
   curl http://localhost:3030

   # Check container IP
   docker inspect my-frontend-app | grep IPAddress
   ```
...
2. **Build Performance**
   ...
   DOCKER_BUILDKIT=1 docker build -t my-frontend-app .
...
3. **Network Issues**
   ...
   docker inspect my-frontend-app | grep -A 20 "NetworkSettings"
   ```
...
docker build -f Dockerfile.dev -t my-frontend-app:dev .
docker run -d -p 3000:3000 -v $(pwd):/app my-frontend-app:dev
```
...
docker build -t my-frontend-app:latest .
docker run -d -p 3030:3030 --name my-frontend-app --restart unless-stopped my-frontend-app:latest
```
...
docker logs -f -t my-frontend-app

# Last 100 lines
docker logs --tail 100 my-frontend-app

# Logs since specific time
docker logs --since="2024-10-24T00:00:00" my-frontend-app
```
...
docker inspect my-frontend-app --format='{{range .State.Health.Log}}{{.Start}} - {{.Output}}{{end}}'

# Container uptime
docker inspect my-frontend-app --format='{{.State.StartedAt}}'
```
...
docker scan my-frontend-app:latest

# Monitor container for suspicious activity
docker logs my-frontend-app | grep -i error
```
...
| **Container** | `my-frontend-app`                   | Docker container name    |
| **Image**     | `my-frontend-app:latest`            | Docker image tag         |
| **Port**      | 3030                                | Application port         |
| **Logs**      | `docker logs my-frontend-app`       | View application logs    |
| **Status**    | `docker ps \| grep my-frontend-app` | Check container status   |
